 ...
 *
 */
public class JogoDeBola extends JFrame {
	...
	private void initComponents() {
		...
		try {
			...
			InputStream input = classLoader.getResourceAsStream("./images/bola.png");
		    idolo = ImageIO.read(input);
		    input = classLoader.getResourceAsStream("./images/cenario2.png");
	...
	private void initBox2D() {
		...
		chaoDef.position.set(larguraMundo / 2, 1f);
		...
		bolaDef.position.set(larguraMundo / 2, 5);
	...
	private void redesenhar() {
		...
		synchronized(gImagem) {
			...
			Vec2 centroImagem = normalizarCoordenadas(bola.getWorldCenter());
	...
	class TrataTeclado implements KeyListener {
		...
		public void keyPressed(KeyEvent key) {
			if (key.getKeyCode() == KeyEvent.VK_SPACE) {
				Vec2 forca = 
						new Vec2(+350.0f * bola.getMass(), 350.0f * bola.getMass());
				
				Vec2 posicao = bola.getWorldCenter().add(new Vec2 (0,3));
				bola.setAwake(true);
				origemX = bola.getTransform().position.x;
				vMax = 0;
