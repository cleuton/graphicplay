 ...
 * 
 */
package com.obomprogramador.games.contact;
...
public class Contact extends JFrame {
	...
	protected String tituloParado = "Contato Box2D - Clique para rodar simulação";
	protected String tituloRodando = "(Rodando) Contato Box2D - Clique para parar simulação";
	...
	private void initBox2D() {
		...
		DadosCorpo dadosChao = new DadosCorpo(chaoTamanho,1);
		chao.createFixture(chaoFixDef);
		chao.setUserData(dadosChao);
		...
		DadosCorpo dadosParedeEsquerda = new DadosCorpo(paredeEsquerdaTamanho,2);
		
		paredeEsquerda.createFixture(paredeEsquerdaFixDef);
		paredeEsquerda.setUserData(dadosParedeEsquerda);
		...
		DadosCorpo dadosParedeDireita = new DadosCorpo(paredeDireitaTamanho,3);
		
		paredeDireita.createFixture(paredeDireitaFixDef);
		paredeDireita.setUserData(dadosParedeDireita);
		
		// Criamos um quadrado dinamico
		BodyDef quadradoDef = new BodyDef();
		quadradoDef.position.set((larguraMundo / 5) * 4, 9);
		...
		DadosCorpo dadosQuadrado = new DadosCorpo(quadradoTamanho,4);
		
		quadrado.createFixture(quadradoFixDef);
		quadrado.resetMassData();
		quadrado.setUserData(dadosQuadrado);
		...
		//bola.setFixedRotation(true); // Comente esta linha para giro livre
	...
	private void update() {
		...
		if (atingiu) {
			...
			task = null;
			simulando = false;
			JOptionPane.showMessageDialog(this, "A bola atingiu o quadrado");
		}
	}
	...
	private Retangulo criarRetangulo(Body body) {
		...
		Vec2 tamanho = ((DadosCorpo) body.getUserData()).tamanho;
	...
	private Polygon criarPoligono(Body body) {
		...
		Vec2 tamanho = (Vec2) ((DadosCorpo)body.getUserData()).tamanho;
